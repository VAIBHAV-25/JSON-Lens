    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-nowrap items-center gap-1 px-2 py-1.5 border-b surface-1 flex-shrink-0 overflow-x-auto scrollbar-none">
        ...
        <Sheet open={shelfOpen} onOpenChange={handleShelfOpenChange}>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className={`gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap transition-all ${
                showShelfHint
            ...
            >
              ...
              {showShelfHint && (
                <span className="hidden md:inline rounded-full bg-emerald-500/15 px-1.5 py-0.5 text-[10px] leading-none">
                  New
                </span>
              )}
        ...
        <Sheet open={transformOpen} onOpenChange={setTransformOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="sm" className="gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap">
              <Wand2 className="w-3.5 h-3.5" />
              <span className="hidden md:inline">Transforms</span>
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-full sm:max-w-md p-0">
            <SheetHeader className="px-5 py-4 border-b">
        ...
        {/* ── Divider + action buttons (only when there's content) ── */}
        {hasInput && (
          <>
            <div className="w-px h-4 bg-border mx-0.5 flex-shrink-0" />
            <Button variant="ghost" size="sm" onClick={handleFormat} disabled={!isValid} className="gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap" title="Pretty print">
              <AlignLeft className="w-3.5 h-3.5" />
              <span className="hidden lg:inline">Format</span>
            </Button>
            <Button variant="ghost" size="sm" onClick={handleMinify} disabled={!isValid} className="gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap" title="Minify JSON">
              <Minimize2 className="w-3.5 h-3.5" />
              <span className="hidden lg:inline">Minify</span>
            </Button>
            <Button variant="ghost" size="sm" onClick={handleCopy} className="gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap" title="Copy to clipboard">
              <Copy className="w-3.5 h-3.5" />
              <span className="hidden lg:inline">Copy</span>
            </Button>
            <Button variant="ghost" size="sm" onClick={handleDownload} className="gap-1 text-xs h-7 px-2 flex-shrink-0 whitespace-nowrap" title="Download as JSON">
              <Download className="w-3.5 h-3.5" />
              <span className="hidden lg:inline">Save</span>
            </Button>
            <Button variant="ghost" size="sm" onClick={handleClear} className="gap-0.5 text-xs h-7 px-2 flex-shrink-0 text-muted-foreground hover:text-destructive" title="Clear">
        ...
        {/* ── Shelf summary (only on very wide screens) ── */}
        {showToolbarShelfSummary && (
          <div className="hidden xl:flex items-center gap-1.5 text-[11px] text-muted-foreground flex-shrink-0">
