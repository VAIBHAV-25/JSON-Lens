...
const TYPE_CFG: Record<string, { bg: string; border: string; accent: string; labelClr: string; valClr: string; typeClr: string }> = {
  object:  { bg: 'rgba(99,102,241,0.07)',  border: 'rgba(99,102,241,0.55)',  accent: '#6366f1', labelClr: '#c7d2fe', valClr: '#818cf8',  typeClr: '#a5b4fc' },
  array:   { bg: 'rgba(59,130,246,0.07)',  border: 'rgba(59,130,246,0.55)',  accent: '#3b82f6', labelClr: '#bfdbfe', valClr: '#60a5fa',  typeClr: '#93c5fd' },
  string:  { bg: 'rgba(16,185,129,0.07)',  border: 'rgba(16,185,129,0.5)',   accent: '#10b981', labelClr: '#a7f3d0', valClr: '#6ee7b7',  typeClr: '#34d399' },
  number:  { bg: 'rgba(245,158,11,0.07)',  border: 'rgba(245,158,11,0.5)',   accent: '#f59e0b', labelClr: '#fef08a', valClr: '#fcd34d',  typeClr: '#fbbf24' },
  boolean: { bg: 'rgba(249,115,22,0.07)',  border: 'rgba(249,115,22,0.5)',   accent: '#f97316', labelClr: '#fed7aa', valClr: '#fb923c',  typeClr: '#fdba74' },
  null:    { bg: 'rgba(107,114,128,0.07)', border: 'rgba(107,114,128,0.45)', accent: '#6b7280', labelClr: '#d1d5db', valClr: '#9ca3af',  typeClr: '#9ca3af' },
};
...
function jsonToTree(v: unknown, label: string, path: string, maxD = 10, d = 0): TreeNode {
  ...
  if (type === 'object' && v !== null) {
    node.children = Object.entries(v as Record<string, unknown>)
      .map(([k, val]) => jsonToTree(val, k, `${path}.${k}`, maxD, d + 1));
  } else if (type === 'array') {
    const arr = v as unknown[];
    node.children = arr.slice(0, 20)
...
export default function JsonGraph() {
  ...
  const stats = useMemo(() => {
    ...
    const maxDepth = nodes.length ? Math.round(Math.max(...nodes.map((n) => n.x)) / LEVEL_W) : 0;
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      setTransform({ x: mouseX - graphX * newScale, y: mouseY - graphY * newScale, scale: newScale });
  ...
  if (!parsedJson) {
    return (
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #060d1f 0%, #0f172a 100%)' }}
      >
        <div
        ...
        <div className="text-center space-y-1">
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '15px', fontWeight: 500 }}>No JSON loaded</p>
        ...
        >
          {Object.entries(TYPE_CFG).map(([type, cfg]) => (
            <span key={type} className="flex items-center gap-1.5">
              <span style={{ width: 8, height: 8, borderRadius: 2, background: cfg.accent, display: 'inline-block' }} />
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(160deg, #060d1f 0%, #0d1327 55%, #0a1020 100%)' }}
    >
      {/* Dot-grid background */}
      <div
      ...
      >
        {[
          { icon: ZoomIn, title: 'Zoom in', action: () => setTransform((t) => ({ ...t, scale: Math.min(3, t.scale * 1.2) })) },
          { icon: ZoomOut, title: 'Zoom out', action: () => setTransform((t) => ({ ...t, scale: Math.max(0.1, t.scale / 1.2) })) },
          { icon: Maximize2, title: 'Fit view', action: fitView },
          { icon: RotateCcw, title: 'Reset view', action: () => setTransform({ x: 60, y: 60, scale: 1 }) },
        ].map(({ icon: Icon, title, action }) => (
          <button
            key={title}
            onClick={() => { setIsZooming(true); action(); if (zoomTimerRef.current) clearTimeout(zoomTimerRef.current); zoomTimerRef.current = setTimeout(() => setIsZooming(false), 200); }}
            title={title}
            className="p-2 rounded-lg transition-all duration-150"
            style={{ color: 'rgba(255,255,255,0.55)' }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.09)'; (e.currentTarget as HTMLButtonElement).style.color = 'rgba(255,255,255,0.9)'; }}
        ...
        <span
          style={{
            ...
            paddingRight: 8, fontFamily: 'monospace', minWidth: 38, textAlign: 'center',
          }}
        >
          ...
          <div className="flex flex-col gap-1.5">
            {[
              ...
              <div key={label} className="flex items-center justify-between">
                ...
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.82)', fontFamily: 'monospace', fontWeight: 600 }}>{value}</span>
              </div>
            ))}
            ...
            {Object.entries(stats.types).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  <span style={{ width: 7, height: 7, borderRadius: 2, background: TYPE_CFG[type]?.accent || '#6b7280', display: 'inline-block', flexShrink: 0 }} />
                  <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>{type}</span>
                </span>
                <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', fontFamily: 'monospace' }}>{count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        {Object.entries(TYPE_CFG).map(([type, cfg]) => (
          <span key={type} className="flex items-center gap-1.5">
            <span style={{ width: 9, height: 9, borderRadius: 3, background: cfg.accent, display: 'inline-block' }} />
            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.42)' }}>{type}</span>
          </span>
        ))}
        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.2)', marginLeft: 4 }}>· Scroll to zoom · Drag to pan</span>
      </div>
        ...
        <svg width="100%" height="100%" className="select-none">
          ...
          <g
            style={{
              ...
              transformOrigin: '0 0',
              transition: isZooming ? 'transform 0.12s cubic-bezier(0.16,1,0.3,1)' : 'none',
            }}
          >
              ...
              return (
                ...
                >
                  ...
                  <text
                    ...
                    opacity={0.85}
                  >
                    {hasChildren ? childLabel : shortValue(node.value, node.type)}
                  </text>
